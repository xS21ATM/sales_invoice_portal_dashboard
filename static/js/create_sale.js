$(document).ready(function () {
    ...
    // Global Variables
    let is_client_user_name = false;
    let is_sales_employee_id = false;
    let is_operation_employee_id = false;
    ...
    */
    ...
    // $('#delivery_last_date2').change(function () {

    //     let a = $('#delivery_last_date2').val();
    //     let b = $('#delivery_last_date3').val();
    //     console.log(a);
    //     console.log(b);
    ...
    /*
        Profile Name onchange fills
    */
    // let json_profiles = $('#json_profiles').val();
    // let profiles = JSON.parse(json_profiles);

    // $('#platform_source').change(function () {
    //     let platform_source = $(this).val();
    //     $('#profile_name').empty();

    //     if (platform_source == 'fiverr') {
    //         let fiverrHtml = ``;
    //         profiles.fiverr.forEach(element => {
    //             fiverrHtml += `<option value="${element.id}">${element.name}</option>\n`;
    //         });
    //         $('#profile_name').append(fiverrHtml);

    //     } else if (platform_source == 'upwork') {
    //         let upworkHtml = ``;
    //         profiles.upwork.forEach(element => {
    //             upworkHtml += `<option value="${element.id}">${element.name}</option>\n`;
    //         });
    //         $('#profile_name').append(upworkHtml);

    //     } else if (platform_source == 'pph') {
    //         let pphHtml = ``;
    //         profiles.pph.forEach(element => {
    //             pphHtml += `<option value="${element.id}">${element.name}</option>\n`;
    //         });
    //         $('#profile_name').append(pphHtml);
    //     }
    // });
    ...
    let all_profiles = [
        ...
        {
            platform_source_id: 2,
            platform_source_name: 'Upwork',
            name: 'Upwork Item 1'
        },
        {
            platform_source_id: 2,
    ...
    */
    ...
    // form_create_sale
    $('#form_create_sale').submit(function (event) {

        let submit_alert = '';
        let is_submit_error = false;

        if (!is_client_user_name) {
            is_submit_error = true;
            submit_alert += 'Client User ID Is Required\n';
        }

        if (!is_sales_employee_id) {
            is_submit_error = true;
            submit_alert += 'Sales Employee Is Required\n';
        }
